...
async def add_knowledge(
    ...
    try:
        success = await vector_store_service.add_texts(texts)
        return {"success": success, "message": f"Added {len(texts)} texts to knowledge base"}
    except Exception as e:
...
async def ai_health_check():
    ...
    try:
        has_groq = bool(ai_service.llm)
        return {
            "status": "healthy",
            "ai_enabled": has_groq,
            "service": "groq" if has_groq else "fallback",
            "capabilities": ["summarize", "analyze", "sentiment", "insights"]
        }
    except Exception as e:
